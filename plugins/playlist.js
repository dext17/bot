...
listSerch.push({title: i.title, description: `Autor: ${i.author.name} / ${i.timestamp}`, rowId: `.yta ${i.url}`})} 
conn.sendMessage(m.chat, listMessage, fake)}
if (command == 'yts') {
for (let i of search.all) {
listSerch.push({title: i.title, description: `Autor: ${i.author.name} / ${i.timestamp}`, rowId: `.ytv ${i.url}`})} 
conn.sendMessage(m.chat, listMessage, fake)} 
} catch (e) {
